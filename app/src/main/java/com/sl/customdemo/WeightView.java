...
//    private int[] redSweep;
    ...
    public void setWeight(float weight, float lowW, float heighW, String weightState, int weightStateCode, float differWeight) {
        this.weightState = weightState;
        this.weightStateCode = weightStateCode;
        this.differWeight = differWeight;
        this.weight = defalutweight = weight;
        this.lowW = lowW;
        this.heighW = heighW;
        isDrawBottom = false;
        maxWeight = 150;
        calculateAngle = calculateAngle(weight, lowW, heighW);
        if (calculateAngle <= everyAngle) {
            linearGradientUtilColor = blueEnd;
        } else if (calculateAngle <= everyAngle * 2 + spaceAngle) {
    ...
    private float calculateAngle(float weight, float lowW, float heighW) {
        if (weight < lowW) {
            endAngle = everyAngle * ((weight / lowW) > 1 ? 1 : (weight / lowW));
        } else if (weight >= heighW) {

            float benchmark = ((weight - heighW) / (maxWeight - heighW)) * everyAngle;
            endAngle = everyAngle * 2 + spaceAngle * 2 + benchmark;

        } else {
            float benchmark = (weight - lowW) / (heighW - lowW);
            endAngle = everyAngle + spaceAngle * 1 + everyAngle * (benchmark > 1 ? 1 : benchmark);
...
//        centerPaint.setTypeface(impact);
        ...
        mBlankRadian = 90f;
        sweepAngle = 360 - mBlankRadian;
        spaceAngle = 12f;
        everyAngle = (sweepAngle - spaceAngle * 2) / 3f;
        startAngle = 90 + mBlankRadian / 2f;
        maxAngle = 360 - mBlankRadian / 2f;
        innerArea = new RectF();
        outerArea = new RectF();
        ...
        weightStrValue = getBigDecimalValue(weight);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        float[] positions = new float[]{0, (everyAngle) / 360f, (everyAngle + spaceAngle) / 360f,
                (everyAngle * 2 + spaceAngle) / 360f, (everyAngle * 2 + spaceAngle * 2f) / 360f, (everyAngle * 3 + spaceAngle * 2f) / 360f};
        ...
        float[] pos2 = new float[]{(everyAngle + mBlankRadian) / 360f, (everyAngle + mBlankRadian + everyAngle) / 360f};
        float[] pos3 = new float[]{(everyAngle + mBlankRadian + everyAngle + spaceAngle) / 360f,
                (everyAngle * 2f + mBlankRadian + everyAngle + spaceAngle) / 360f};
...
//        getShader();
    ...
    private void drawCenterText(Canvas canvas) {
        ...
        //画底部的值
        if (weight > 0 && isDrawBottom) {
            textPaint.setTextSize(bottomTextSize);

            if (baseBottomY == 0) {
                float arcRadius = mHeight / 2f;
                baseBottomY = (int) (mHeight / 2f + Math.cos(Math.toRadians(mBlankRadian / 2f)) * arcRadius);
            }
            if (weightStateCode == 0) {
                textPaint.setColor(stateNormalColor);
                canvas.drawText(weightState, mWidth / 2f, baseBottomY, textPaint);
            ...
            canvas.drawText(weightState + differWeight + unit, mWidth / 2f, baseBottomY, textPaint);
    ...
    private void drawBgArc(Canvas canvas) {
        ...
        canvas.save();
        canvas.rotate(-(everyAngle - (180f - mBlankRadian) / 2f) - spaceAngle, mWidth / 2f, mHeight / 2f);
        mPaint.setShader(mBgSweepGradient1);
        canvas.drawArc(rectF, spaceAngle, everyAngle, false, mPaint);
        mPaint.setShader(mBgSweepGradient2);
        canvas.drawArc(rectF, everyAngle + mBlankRadian + spaceAngle, everyAngle, false, mPaint);
        mPaint.setShader(mBgSweepGradient3);
        canvas.drawArc(rectF, everyAngle * 2f + mBlankRadian + spaceAngle + spaceAngle, everyAngle, false, mPaint);
        canvas.restore();

    }

    private void getShader() {
        positions[1] = (18 * 7.5f + calculateAngle) / 360f;
        if (calculateAngle <= everyAngle) {
            sweepGradient = new SweepGradient(mWidth / 2f, mHeight / 2f, new int[]{blueEnd0, blueEnd}, positions);
            mPathPaint.setShader(sweepGradient);
        } else if (calculateAngle <= everyAngle * 2 + spaceAngle) {
            sweepGradient = new SweepGradient(mWidth / 2f, mHeight / 2f, new int[]{greenEnd0, greenEnd}, positions);
            mPathPaint.setShader(sweepGradient);
        } else {
            sweepGradient = new SweepGradient(mWidth / 2f, mHeight / 2f, new int[]{redEnd00, redEnd}, positions);
            mPathPaint.setShader(sweepGradient);
        }
    }
    ...
    private void initAnimator() {
        ...
        mValueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                endAngle = (value * (calculateAngle));
                weight = defalutweight * value;
                weightStrValue = getBigDecimalValue(weight);
                invalidate();
            }
        });
